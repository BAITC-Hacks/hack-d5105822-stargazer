...
│   └── processed/          # очищенные/ресемплированные в час parquet
├── src/
│   ├── config.py             # загрузка config/turbines.yaml
│   ├── data/                 # schema.py, io.py, resample.py — история турбин
│   ├── weather/               # schema.py, cache.py, client.py — Open-Meteo клиент
│   ├── features/            # feature engineering (следующий шаг)
│   ├── models/               # обучение, инференс, power curve baseline
...
3. **Сборка обучающего датасета**: сопоставить историческую выработку с архивным прогнозом погоды на соответствующий lead time (а не с фактической погодой) — это готовит модель к тем же условиям, что и на инференсе.
