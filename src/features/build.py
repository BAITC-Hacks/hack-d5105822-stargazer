...
from src.features import engineering, schema, weather_features
...
def assemble_turbine_dataset(turbine_id: int) -> tuple[pd.DataFrame, dict]:
    ...
    for lead_hours in schema.LEAD_HOURS_OPTIONS:
        ...
        lags = engineering.lag_features(merged["valid_time"], lead_hours, power_clean)
        merged = merged.merge(lags, on="valid_time", how="left")

        merged = engineering.engineer_physical_features(merged)
        merged = engineering.engineer_time_features(merged)
